...
const BubbleSortVisualizer = () => {
    ...
    const [currIndexes, setCurrIndexes] = useState<{ i: number, j: number }>({ i: -1, j: -1 });
    ...
    const handleRandomizer = () => {
        const minLength = 5;
        const maxLength = 10;
        const randomLength = Math.floor(Math.random() * (maxLength - minLength + 1) + minLength); // random length from 5 to 10
        const randomNums = Array.from({length: randomLength}, () => Math.floor(Math.random() * 99) + 1);
        setArray(randomNums);
            ...
            >
                {array.map((num, index) => (
                    <div key={index} style={{
                        ...
                        backgroundColor: 
                            ((index === currIndexes.j) && (isSorting))? "orange"
                            : ((index === currIndexes.j + 1) && (isSorting))? "brown"
                            : (!isSorting)? "grey"
                            : "black",
                        color: "white",
                        display: "flex",
